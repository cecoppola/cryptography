 ...
 *
 * Included across the engine (ntt_kernel.hip, ntt_mul.hip, garner.hip,
 * main.hip, test.hip) and transitively via bigint.h by the arith layer.
 * Centralises U256 and GarnerConsts so the translation units linked into
 * one binary share a single typedef (no duplicate-typedef errors).
 */
