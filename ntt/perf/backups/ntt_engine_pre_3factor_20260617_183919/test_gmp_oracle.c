 ...
 *
 * GMP dev header (gmp.h) is not installed on this box, but libgmp.so is — so we
 * declare the few __gmpz_* prototypes we use directly. mp_limb_t is 64-bit
 * (unsigned long, LP64) matching GMP's build here and our LIMB_BITS=64 layout,
 * so mpz_import/export with size=8, order=-1, endian=0 maps 1:1 to BigInt.limbs.
 *
 * Usage:  test_gmp_oracle <operand_limbs> [seed]
...
{
    /* ntt_bigint_mul uses a FIXED N=2^log_n set by ntt_mul_init (see ntt_mul.hip
     * line 542: log_n = g_nms.log_n). So we init at the log_n under test and size
     * operands to ~N/2 limbs each (product ~N, must stay <= N to avoid the cyclic
     * NTT wrapping). arg1 = log_n to test (default 22 = the suspected-broken one). */
